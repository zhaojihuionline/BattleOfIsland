...
namespace QFramework.UI
{
    ...
    public class PageUseType2ChoiceView : BagPageViewBase
    {
        ...
        public override void RefreshData(BagItemData itemData)
        {
            ...
            if (itemConfig == null)
            {
                Debug.LogWarning($"PageUseType2ChoiceView: 无法获取物品配置 ItemId={itemData.ItemId}");
                ResetView();
                return;
            }
            ...
            if (itemConfig.RewardID > 0)
            {
                ...
                {
                    // TODO: 根据 rewardConfig.RewardDetail 显示可选择的奖励列表
                }
            }
        }
        ...
        private void OnConfirmButtonClicked()
        {
            ...
            Debug.Log($"PageUseType2Choice: 确认使用物品 {currentItemData.ItemId}");
            // 发送使用物品的命令，需要传递选中的奖励ID列表
            // this.SendCommand(new UseBagItemCommand(currentItemData.BagId, 1, selectedRewardIds: selectedIds));
        }
    }
}
