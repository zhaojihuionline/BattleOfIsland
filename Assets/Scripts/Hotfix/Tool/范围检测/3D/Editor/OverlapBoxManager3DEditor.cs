...
#if UNITY_EDITOR
[CustomEditor(typeof(OverlapBoxManager3D))]
public class OverlapBoxManager3DEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        var manager = (OverlapBoxManager3D)target;

        EditorGUILayout.LabelField("3D碰撞检测管理器", EditorStyles.boldLabel);
        ...
        showManagedBoxes = EditorGUILayout.Foldout(showManagedBoxes, $"管理的3D检测器 ({manager.GetManagedBoxCount()})", true);
        if (showManagedBoxes)
        {
        ...
        // 统计信息
        showStats = EditorGUILayout.Foldout(showStats, "检测器统计", true);
        if (showStats)
        {
            DrawDetectorStats(manager);
        }
    ...
    private void DrawDetectorStats(OverlapBoxManager3D manager)
    {
        ...
        foreach (var box in manager.managedOverlapBoxes)
        {
            if (box != null)
            {
                if (box.isActiveAndEnabled) activeCount++;
        ...
        EditorGUILayout.LabelField($"活跃检测器: {activeCount}/{manager.GetManagedBoxCount()}");
        EditorGUILayout.LabelField($"立方体: {cubeCount}");
        EditorGUILayout.LabelField($"圆柱体: {cylinderCount}");
        EditorGUILayout.LabelField($"球体: {sphereCount}");
    ...
    private void DrawObjectLists(OverlapBoxManager3D manager)
    {
        // 内环对象
