...
/// <summary>
/// 2D统一碰撞检测管理器 - 管理多个OmnipotentOverlapBox
/// </summary>
public class OverlapBoxManager : MonoBehaviour
{
    [Header("检测设置")]
    ...
    [Header("管理器设置")]
    public List<OmnipotentOverlapBox> managedOverlapBoxes = new List<OmnipotentOverlapBox>();
    ...
    /// <summary>
    /// 收集当前物体下所有的OmnipotentOverlapBox
    /// </summary>
    public void CollectAllOverlapBoxes()
    {
        managedOverlapBoxes.Clear();

        // 获取当前物体和所有子物体中的OmnipotentOverlapBox组件
        var allBoxes = GetComponentsInChildren<OmnipotentOverlapBox>(true);
        managedOverlapBoxes.AddRange(allBoxes);

        Debug.Log($"已收集 {managedOverlapBoxes.Count} 个2D碰撞检测器");
    }
    ...
    public void DetectAllCollisions()
    {
        ...
        Debug.Log($"2D检测完成: {activeBoxCount}个检测器, {allInnerRingObjects.Count}个内环对象, {allOuterRingObjects.Count}个外环对象");
    }

    /// <summary>
    /// 使用管理器的设置对单个Box进行检测
    /// </summary>
    private void DetectWithBox(OmnipotentOverlapBox box)
    {
        // 保存检测器原有设置
        var originalLayers = box.detectionLayers;
        var originalTags = new TagMask2D();
    ...
    /// 复制标签掩码设置
    /// </summary>
    private void CopyTagMask(TagMask2D source, TagMask2D destination)
    {
    ...
    public string GetDetectorStats()
    {
        ...
        return $"矩形: {rectangleCount}, 扇形: {sectorCount}";
    }
}